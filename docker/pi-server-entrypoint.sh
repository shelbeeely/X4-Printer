...
#
# Gated on FOCUSINK_AUTO_TLS=1 (set only by the production docker-compose.yml
...
# $FOCUSINK_TLS_CERT, so this only ever runs once per persistent /data volume
# -- re-run pi-server/tools/gen_selfsigned_cert.py by hand (see its own
# header comment) if the Pi's LAN IP changes later.
set -eu

if [ "${FOCUSINK_AUTO_TLS:-}" = "1" ]; then
  cert="${FOCUSINK_TLS_CERT:-/data/tls/server.crt}"
  key="${FOCUSINK_TLS_KEY:-/data/tls/server.key}"
