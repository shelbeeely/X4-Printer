...
logger = logging.getLogger("focusink.config")
...
class Config:
    ...
    data_dir: Path = field(default_factory=lambda: Path(_env_str("FOCUSINK_DATA_DIR", "/var/lib/focusink-server")))
    ...
    ipp_host: str = field(default_factory=lambda: _env_str("FOCUSINK_IPP_HOST", "0.0.0.0"))
    ipp_port: int = field(default_factory=lambda: _env_int("FOCUSINK_IPP_PORT", 6310))
    printer_name: str = field(default_factory=lambda: _env_str("FOCUSINK_PRINTER_NAME", "Focusink"))
    render_dpi: int = field(default_factory=lambda: _env_int("FOCUSINK_RENDER_DPI", 150))

    # X4 sync API (device <-> Pi). TLS + bearer token, see docs/protocol.md.
    sync_host: str = field(default_factory=lambda: _env_str("FOCUSINK_SYNC_HOST", "0.0.0.0"))
    sync_port: int = field(default_factory=lambda: _env_int("FOCUSINK_SYNC_PORT", 8443))
    tls_cert: Path = field(default_factory=lambda: Path(_env_str("FOCUSINK_TLS_CERT", "/var/lib/focusink-server/tls/server.crt")))
    tls_key: Path = field(default_factory=lambda: Path(_env_str("FOCUSINK_TLS_KEY", "/var/lib/focusink-server/tls/server.key")))

    # X4 panel geometry — what xtc_writer renders pages to. 800x480 (X4).
    panel_width: int = field(default_factory=lambda: _env_int("FOCUSINK_PANEL_WIDTH", 800))
    panel_height: int = field(default_factory=lambda: _env_int("FOCUSINK_PANEL_HEIGHT", 480))
    ...
    cups_queue: str = field(default_factory=lambda: _env_str("FOCUSINK_CUPS_QUEUE", ""))
    lp_binary: str = field(default_factory=lambda: _env_str("FOCUSINK_LP_BINARY", "lp"))

    # Cloud relay (optional; empty relay_url disables the poller thread).
    relay_url: str = field(default_factory=lambda: _env_str("FOCUSINK_RELAY_URL", ""))
    relay_account_id: str = field(default_factory=lambda: _env_str("FOCUSINK_RELAY_ACCOUNT_ID", ""))
    relay_account_token: str = field(default_factory=lambda: _env_str("FOCUSINK_RELAY_ACCOUNT_TOKEN", ""))
    relay_poll_interval_seconds: int = field(default_factory=lambda: _env_int("FOCUSINK_RELAY_POLL_INTERVAL", 20))
    relay_allow_document_sync: bool = field(default_factory=lambda: _env_bool("FOCUSINK_RELAY_ALLOW_DOCUMENT_SYNC", False))
    ...
    retention_days: int = field(default_factory=lambda: _env_int("FOCUSINK_RETENTION_DAYS", 30))
    ...
    admin_host: str = field(default_factory=lambda: _env_str("FOCUSINK_ADMIN_HOST", "0.0.0.0"))
    admin_port: int = field(default_factory=lambda: _env_int("FOCUSINK_ADMIN_PORT", 8090))
    admin_password: str = field(default_factory=lambda: _env_str("FOCUSINK_ADMIN_PASSWORD", ""))

    log_level: str = field(default_factory=lambda: _env_str("FOCUSINK_LOG_LEVEL", "INFO"))
